...
export const loader = async ({
  ...
  // Set selected oprtions from the query string
  searchParams.forEach((value, name) => {
    selectedOptions.push({name, value});
  });

  if (!handle) {
    throw new Response('missing handle param', {status: 404});
  }
  ...
  return json({shop, product, selectedVariant});
};

export default function ProductHandle() {
  const {shop, product, selectedVariant} = useLoaderData<typeof loader>();
  return (
    <section className="w-full gap-4 md:gap-8 grid px-6 md:px-8 lg:px-12">
      <div className="grid items-start gap-6 lg:gap-20 md:grid-cols-2 lg:grid-cols-3">
        <div className="grid md:grid-flow-row md:p-0 md:overflow-x-hidden md:grid-cols-2 lg:grid-cols-3">
